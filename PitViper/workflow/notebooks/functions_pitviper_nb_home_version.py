...
def show_read_count_distribution(path_qc, width=800, height=400):
#     path_qc = "./data/{token}/screen.count.txt".format(token=token)
#     path_qc = "/mnt/d/pitviper_results/CRISPRi_M-07e/screen.count.txt"
...
def show_mapping_qc(path):
#     path_qc = "./data/{token}/screen.countsummary.txt".format(token=token)
#     path_qc = "/mnt/d/pitviper_results/CRISPRi_M-07e/screen.countsummary.txt"
    ...
    def sgRNA_accross_conditions(feature, baseline):
        ...
        def sgRNA_accross_conditions(treatment):
            ...
            def on_button_clicked(b):
                ...
                print(info[treatment + '|control_count'])
                print(baseline, treatment)

                info[[baseline+'_1',baseline+'_2', baseline+'_3']] = info[treatment + '|control_count'].str.split("/",expand=True,)
                
                info[[treatment+'_1',treatment+'_2', treatment+'_3']] = info[treatment + '|treatment_count'].str.split("/",expand=True,)
...
def crisphiermix_feature_accros_conditions(crm, baseline, feature, fdr_cutoff, comparisons, treatments):
    ...
    plot = alt.Chart(info).mark_circle(size=60).mark_point(
        ...
        size=100,
                ...
                title=feature + " score versus baseline (CRISPhieRmix)",
                width=100
        )
    return plot
        ...
        def test_plotting(treatment, non_sig, sig):
            ...
            def on_button_clicked(b):
                chart = alt.Chart(source).mark_circle(size=60).encode(
                    ...
                    tooltip=['Gene', 'sgRNA', treatment + '|z', treatment + '|fdr', 'significant'],
                    color=alt.Color('significant', scale=alt.Scale(domain=domain, range=range_), legend=alt.Legend(title="Significativity:"))
                ).properties(width=width, height=height).interactive()
