...
class Design:
    ...
    def createSamplesSummary(self):
        ...
        for col in self.conditions:
            ...
            self.samples_dict[col]["replicates"] = self.sample_sheet.loc[
                self.sample_sheet.condition == col
            ]["replicate"].values
    ...
    def create_design_matrix(self):
        ...
        self.fillConditions()
        self.fillNan()
        self.design_matrix = self.design_matrix.loc[
            :, (self.design_matrix != 0).any(axis=0)
        ]
...
def readSamples(file, directory, control, treatment, dryrun):
    ...
    file_name = directory + "{treatment}_vs_{control}_design_matrix.txt".format(
