...
def mapEnhancerToGene(
    ...
    for line in enhancerTable[6:]:
        ...
        distalLoci = tssCollection.getOverlap(
            ROSE_utils.makeSearchLocus(enhancerLocus, 50000000, 50000000), "both"
        )
        distalGenes = []
        ...
        if len(allEnhancerGenes) == 0:
            ...
            closestGene = startDict[allEnhancerGenes[distList.index(min(distList))]][
                "name"
            ]
        ...
        if byRefseq:
            newEnhancerLine.append(
                ",".join(ROSE_utils.uniquify([x for x in overlappingGenes]))
            )
            newEnhancerLine.append(
                ",".join(ROSE_utils.uniquify([x for x in proximalGenes]))
            )
            ...
            newEnhancerLine.append(
                ",".join(
                    ROSE_utils.uniquify([startDict[x]["name"] for x in proximalGenes])
                )
            )
            closestGene = startDict[allEnhancerGenes[distList.index(min(distList))]][
                "name"
