...
class Locus:
    ...
    def overlaps(self, otherLocus):
        if self.chr() != otherLocus.chr():
            ...
            self._sense == "."
            or otherLocus.sense() == "."
            or self.sense() == otherLocus.sense()
        ):
            return False
    ...
    def __str__(self):
        return (
            self.chr() + "(" + self.sense() + "):" + "-".join(map(str, self.coords()))
        )
        ...
        for locus in locusList:
            ...
            if oldCollection.hasLocus(locus):
                ...
                overlappingLoci = oldCollection.getOverlap(
                    Locus(
                        locus.chr(),
                        locus.start() - stitchWindow,
                        locus.end() + stitchWindow,
                        locus.sense(),
                        locus.ID(),
                    ),
                    sense,
...
def makeTSSLocus(gene, startDict, upstream, downstream):
    ...
    if startDict[gene]["sense"] == "-":
        return Locus(
            startDict[gene]["chr"], start - downstream, start + upstream, "-", gene
        )
    else:
        return Locus(
            startDict[gene]["chr"], start - upstream, start + downstream, "+", gene
        )
...
def makeSearchLocus(locus, upSearch, downSearch):
    if locus.sense() == "-":
        ...
        searchLocus = Locus(
            locus.chr(),
            locus.start() - upSearch,
            locus.end() + downSearch,
            locus.sense(),
            locus.ID(),
        )
    return searchLocus
...
class Bam:
    ...
    def getRawReads(
        self, locus, sense, unique=False, includeJxnReads=False, printCommand=False
    ):
        """
    ...
    def readsToLoci(self, reads, IDtag="sequence,seqID,none"):
        ...
        for read in reads:
            ...
            if read[5].count("N") == 1:
                ...
                [first, gap, second] = [
                    int(x)
                    for x in [x for x in re.findall(numPattern, read[5]) if len(x) > 0]
                ][0:3]
                if IDtag == "sequence":
