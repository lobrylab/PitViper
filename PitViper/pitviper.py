...
def run_pitviper(configfile, dry_run, jobs, notebook):
    if notebook != "":
        ...
        nb_opt = ""
    if dry_run:
        cmd = "snakemake -s workflow/Snakefile -n --configfile {configfile} --use-conda --cores {jobs}".format(
            configfile=configfile, jobs=jobs
        )
    elif not dry_run:
        cmd = "snakemake -s workflow/Snakefile --configfile {configfile} --use-conda --cores {jobs} {nb_opt}".format(
            configfile=configfile, jobs=jobs, nb_opt=nb_opt
        )
    print("Command:", cmd)
    os.system(cmd)
...
def main(configfile, dry_run, jobs, notebook):
    try:
        ...
        print(
            "Error: cannot open and read configuration file {configfile}".format(
