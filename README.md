...
`run.sh` use two arguments: installer in position 1 ("mamba" or "conda") and mode in position 2 ("yaml" or "lock"). All 4 combinations are possible.

```bash
# Default: install dependencies with mamba and use the YAML configuration file. 
$ ./run.sh mamba yaml  # same than behaviour than ./run.sh alone.

# Install dependencies with conda (default) and use the lock YAML configuration file (generated by conda-lock).
$ ./run.sh conda lock
```
